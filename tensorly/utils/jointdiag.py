...
def jointdiag(
    ...
    for k in range(max_iter):
        ...
        for p, q in combinations(range(D), 2):
            ...
            pvec = tl.tensor(X[p, :, :])
            X[p, :, :] = X[p, :, :] * tl.cosh(yk) - X[q, :, :] * tl.sinh(yk)
            X[q, :, :] = -pvec * tl.sinh(yk) + X[q, :, :] * tl.cosh(yk)
            ...
            # Update Q_total
            pvec = Q_total[:, p].copy()
            Q_total[:, p] = Q_total[:, p] * tl.cosh(yk) + Q_total[:, q] * tl.sinh(yk)
            Q_total[:, q] = pvec * tl.sinh(yk) + Q_total[:, q] * tl.cosh(yk)
            ...
            Q_total[:, q] = pvec * tl.sin(theta_k) + Q_total[:, q] * tl.cos(theta_k)
